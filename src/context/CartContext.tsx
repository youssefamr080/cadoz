    ...
    if (existingItemIndex > -1) {
      ...
      return state.map((cartItem, index) => 
       index === existingItemIndex
        ? { ...cartItem, quantity: Math.min(newQuantity, stockLimit) }
        : cartItem
      );
    } else {
...
export const CartProvider = ({ children }: { children: ReactNode }) => {
  ...
  const getShippingFees = useCallback(() => {
   ...
   const promoFreeShipping = promoCode.isValid && promoDetails && ('freeShipping' in promoDetails ? promoDetails.freeShipping : false);
