...
const SearchBar: React.FC<SearchBarProps> = ({ onProductSelect, className = "" }) => {
    ...
    // ✨ استخدام التعليقات الانجليزية لسهولة التواصل مع المطورين العالميين
    ...
    // Enhanced Fuse.js options for better search results
    ...
    // Update search results with debounce for performance
    const updateSearchResults = useCallback(
        debounce((value: string) => {
            if (fuse.current && value.length > 1) {
                ...
                setTimeout(() => {
                    const searchResults: Product[] = fuse.current!
                        .search(value)
        ...
        [setResults] // Add setResults as a dependency
    );
    ...
    // Apply term from recent searches - renamed to avoid the Hook-like naming
                            ...
                            {!isLoading && query.length > 1 && results.length === 0 && (
                                <div className="py-12 flex flex-col items-center justify-center text-gray-500">
                                    <svg className="h-12 w-12 mb-4 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        ...
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 14l2 2 4-4" style={{ opacity: 0 }} />
                                    </svg>
