...
const GiftStepChocolates: React.FC = () => {
  ...
  const chocolateGroups = useMemo<GroupedChunks>(() => {
    const groups: { [key: string]: GiftOption[] } = chocolates.reduce((acc, chocolate) => {
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.4 }}
    >
      <GiftSectionHeader title="اختر الشوكولاتة المفضلة" icon={<Heart className="w-6 h-6" />} variant="accent" />
