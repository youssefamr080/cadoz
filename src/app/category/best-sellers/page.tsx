...
const BestSellersPage = () => {
  ...
  const { sortOption, showSortOptions, setShowSortOptions, handleSortChange } = useSortOptions("popularity")
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center">
        <LoadingSpinner message="جاري تحميل المنتجات الرائجة..." />
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="pb-16">
        {/* بانر الصفحة */}
        <div className="relative h-48 md:h-64 lg:h-80 bg-gradient-to-r from-amber-400 to-orange-600 overflow-hidden">
          ...
          <div className="container mx-auto px-4 h-full flex flex-col justify-center items-center text-white relative z-10">
            ...
            >
              ...
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-2">المنتجات الرائجة</h1>
                  ...
                  <div className="py-1">
                    <button
                      className={`w-full text-right px-4 py-2 text-sm ${
                        sortOption === "popularity" ? "bg-slate-100 text-amber-600" : "text-slate-700 hover:bg-slate-50"
                      }`}
                      onClick={() => handleSortChange("popularity")}
                    >
                      <div className="flex items-center justify-between">
                        <span>الأكثر شعبية</span>
                        {sortOption === "popularity" && <Check className="w-4 h-4" />}
                      </div>
                    </button>
                    <button
                      className={`w-full text-right px-4 py-2 text-sm ${
                        sortOption === "price_asc" ? "bg-slate-100 text-amber-600" : "text-slate-700 hover:bg-slate-50"
                      }`}
                    ...
                    <button
                      className={`w-full text-right px-4 py-2 text-sm ${
                        sortOption === "price_desc" ? "bg-slate-100 text-amber-600" : "text-slate-700 hover:bg-slate-50"
                      }`}
                    ...
                    <button
                      className={`w-full text-right px-4 py-2 text-sm ${
                        sortOption === "newest" ? "bg-slate-100 text-amber-600" : "text-slate-700 hover:bg-slate-50"
                      }`}
                ...
                <motion.div key={product.id} variants={fadeInUp}>
                  ...
                  >
                    <div className="relative pt-[100%] overflow-hidden bg-slate-100">
                      ...
                      />
                      {product.old_price && (
                        <div className="absolute top-2 left-2 bg-gradient-to-r from-amber-400 to-orange-600 text-white py-0.5 px-2 text-xs font-semibold rounded-full shadow-sm">
                          {Math.round(((product.old_price - product.price) / product.old_price) * 100)}%
                        </div>
                      )}
                    </div>
                    <div className="p-3 flex-1 flex flex-col">
                      <h3 className="font-medium text-sm line-clamp-2 mb-1 flex-1 group-hover:text-amber-600 transition-colors">
                        {product.name}
                      </h3>
